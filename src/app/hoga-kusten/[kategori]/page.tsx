...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const region = REGIONS.hogakusten
  ...
  const tom = (await getPlacesForRegionCategory('hogakusten', kategori)).length === 0
  return {
...
export default async function Page({ params }: Props) {
  ...
  return <RegionCategoryPage regionKey="hogakusten" categoryKey={kategori} />
}
