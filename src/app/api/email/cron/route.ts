...
async function handle(req: Request) {
  ...
  const { data: day3Candidates } = await service
  ...
  let day3Sent = 0
  let day3Errors = 0
  if (day3Candidates && day3Candidates.length > 0) {
    const { data: alreadySentDay3 } = await service
      .from('email_log')
      .select('email')
      .eq('template', 'day3_newsletter')
      .in('email', day3Candidates.map((c: { email: string }) => c.email))
    const sentDay3 = new Set((alreadySentDay3 ?? []).map((r: { email: string }) => r.email as string))
