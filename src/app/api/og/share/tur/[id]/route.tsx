...
export async function GET(
  ...
  try {
    ...
    console.log('[share-og] A id=', id)

    const supabase = createClient()
    console.log('[share-og] B supabase created')
    ...
    console.log('[share-og] C trip=', !!trip, 'err=', tripErr?.message)

    const { data: userRow } = trip
      ? await supabase.from('users').select('username').eq('id', trip.user_id).single()
      : { data: null }

    console.log('[share-og] D username=', userRow?.username)
