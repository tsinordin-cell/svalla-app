...
export default function MeddelandenPage() {
  ...
  // Derived lists — beräknas i render, inga IIFEs
  const activeConvs = convs.filter(c =>
    c.status === 'active' || (c.status === 'request' && c.created_by === me)
  )
  const requestConvs = convs.filter(c =>
    c.status === 'request' && c.created_by !== me
  )
  const visibleConvs = tab === 'requests' ? requestConvs : activeConvs
        ...
        <div style={{ maxWidth: 520, margin: '0 auto', padding: '0 16px', borderBottom: '1px solid rgba(10,123,140,0.08)' }}>
          <div style={{ display: 'flex', gap: 4 }}>
            <TabButton active={tab === 'active'} onClick={() => setTab('active')}>
              Aktiva{activeConvs.length > 0 && (
                <span style={{ color: 'var(--txt3)', fontWeight: 600 }}> · {activeConvs.length}</span>
              )}
            </TabButton>
            <TabButton active={tab === 'requests'} onClick={() => setTab('requests')}>
              ...
              {requestConvs.length > 0 && (
                <span style={{
                  ...
                  {requestConvs.length > 9 ? '9+' : requestConvs.length}
                </span>
              )}
            </TabButton>
          </div>
      ...
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '8px 0' }}>
        ...
        {error && (
          <div style={{ padding: '60px 20px', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--txt3)', fontSize: 14, marginBottom: 16 }}>Kunde inte ladda meddelanden. Försök igen.</p>
            <button
              onClick={() => me && load(me)}
              style={{ padding: '10px 20px', borderRadius: 12, background: '#1e5c82', color: '#fff', fontWeight: 700, fontSize: 13, border: 'none', cursor: 'pointer' }}
            >
              Försök igen
            </button>
          </div>
        ...
        {/* Konversationslista */}
        {!loading && !error && me && visibleConvs.map(c => (
          <ConvItem key={c.id} c={c} me={me} />
        ))}
      </div>
...
function ConvItem({ c, me }: { c: ConvRow; me: string }) {
  ...
  return (
    <Link href={`/meddelanden/${c.id}`} style={{ textDecoration: 'none' }}>
      <div style={{
