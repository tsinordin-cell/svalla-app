...
export default function PlaneraShare({ routeId, startName, endName }: Props) {
 const [copied, setCopied] = useState(false)
 const [hasNativeShare, setHasNativeShare] = useState(false)
 const url = `https://svalla.se/planera/${routeId}`
 const routeLabel = startName && endName ? `${startName} → ${endName}` : 'den här skärgårdsrutten'
 const shareText = `Kolla in ${routeLabel} på Svalla`
 ...
 try {
 await navigator.share({ title: 'Rutt på Svalla', text: shareText, url })
 ...
 <div style={{
 fontSize: 13, fontWeight: 700, color: 'var(--txt)', marginBottom: 10,
 ...
 display: 'flex', alignItems: 'center', gap: 6,
 padding: '9px 14px', borderRadius: 10,
 background: 'rgba(10,123,140,0.08)', border: '1px solid rgba(10,123,140,0.15)',
 color: 'var(--sea)', fontSize: 12, fontWeight: 700, textDecoration: 'none',
 }}
 >
 <Icon name="mail" size={14} stroke={2} />
 SMS
