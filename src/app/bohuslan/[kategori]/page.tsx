...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const tom = (await getPlacesForRegionCategory('bohuslan', kategori)).length === 0
  return {
...
export default async function Page({ params }: Props) {
  ...
  return <RegionCategoryPage regionKey="bohuslan" categoryKey={kategori} />
}
