 ...
 *
 * Vanligast är `refresh_token_already_used`: två flikar (eller en flik och en
 * service worker) förnyar samma token samtidigt, den ena vinner och den andra
 * får 400. Sessionen är död och enda vägen framåt är att glömma den.
 */
 ...
 *
 * VARFÖR (2026-08-12): komponenterna anropade `supabase.auth.getUser().then(…)`
 * utan .catch. När en refresh-token var förbrukad avvisades löftet, avvisningen
 * togs aldrig om hand och felet bubblade upp till app/error.tsx — "Något gick
 * fel" på en helt publik sida som /oar. NotificationBell och MessageBell ligger
 * i layouten, så det drabbade VARJE sida, men bara inloggade besökare, vilket
 * gjorde det svårt att återskapa utloggad.
 *
 * Returformen är identisk med getUser() så anropen kan bytas rakt av. Vid död
 * session rensas den lokala sessionen så nästa sidladdning börjar om rent.
 */
