...
export async function getThreadsByCategory(categoryId: string, page = 0): Promise<ForumThread[]> {
  ...
  try {
    ...
    // Undvik .in('id', []) med tom array — PostgREST-beteende är odefinierat
    if (allUserIds.length === 0) return data as ForumThread[]
...
// ─── Formatters ──────────────────────────────────────────────────────────────
// Re-exporterat från forum-utils.ts för bakåtkompatibilitet (server-imports).
// 'use client'-komponenter ska importera direkt från '@/lib/forum-utils'.
export { formatForumDate } from './forum-utils'
