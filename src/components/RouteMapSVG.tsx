...
export default function RouteMapSVG({
  ...
  //     utan rörelse. Visa ändå en platskarta (Z=13) med bara en prick
  //     i stället för att returnera null och lämna en svart ruta.
  const latSpan = maxLat - minLat
  const lngSpan = maxLng - minLng
  if (latSpan < 0.0005 && lngSpan < 0.0005) {
    const Z0 = 13
    const lngToTileX0 = (lng: number) => ((lng + 180) / 360) * Math.pow(2, Z0)
    const latToTileY0 = (lat: number) => {
      const r = (lat * Math.PI) / 180
      return ((1 - Math.log(Math.tan(r) + 1 / Math.cos(r)) / Math.PI) / 2) * Math.pow(2, Z0)
    }
    const cx0 = lngToTileX0(points[0].lng)
    const cy0 = latToTileY0(points[0].lat)
    const tx0b = Math.floor(cx0) - 1, ty0b = Math.floor(cy0) - 1
    const gl0 = w / 2 - (cx0 - tx0b) * 256
    const gt0 = h / 2 - (cy0 - ty0b) * 256
    const [px, py] = [w / 2, h / 2]
    return (
      <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', background: '#d4e6ef' }}>
        <div style={{ position: 'absolute', left: gl0 - 256, top: gt0 - 256, width: 768, height: 768 }}>
          {[0, 1, 2].map(dy => [0, 1, 2].map(dx => (
            <div key={`${dx}_${dy}`} style={{ position: 'absolute', left: dx * 256, top: dy * 256, width: 256, height: 256 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={`https://tile.openstreetmap.org/${Z0}/${tx0b + dx}/${ty0b + dy}.png`} alt="" loading="lazy" decoding="async" className="route-map-tile" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' }} />
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={`https://tiles.openseamap.org/seamark/${Z0}/${tx0b + dx}/${ty0b + dy}.png`} alt="" loading="lazy" decoding="async" className="route-map-tile" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block', pointerEvents: 'none' }} />
            </div>
          )))}
        </div>
        <svg viewBox={`0 0 ${w} ${h}`} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
          <circle cx={px} cy={py} r={7} fill="#e8520a" stroke="white" strokeWidth="2.5" />
        </svg>
      </div>
    )
  }
